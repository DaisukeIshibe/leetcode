...
def top_three_salaries(employee: pd.DataFrame, department: pd.DataFrame) -> pd.DataFrame:
	...
	# Function to get top three unique salaries in each department
	def get_top_earners(group):
		top_salaries = group['salary'].drop_duplicates().nlargest(3)
		return group[group['salary'].isin(top_salaries)]
	
	# Apply the function to each department group
	result = merged.groupby('name_dept').apply(get_top_earners).reset_index(drop=True)
